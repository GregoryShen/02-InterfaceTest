...
上一次，我们新建了 `core/rest_client.py`这个文件，并且可以使用其中的 RestClient 类来收发 HTTP 请求，这一小节，我们接着上次的 rest 请求客户端，向其中增加登录GitHub的功能。
...
这里, 通过修改 RestClient 类的初始化方法, 添加了三个参数, username, password和token. 当 username 和password 不为空时使用用户名密码登录, 向 requests session 中添加auth 信息. 当 token不为空时使用token 登录, 向requests session 的headers里的 Authorization 字段添加token信息.
...
除了这些简单接口的封装, 存放在 API 目录下外, 我们还会实现更业务化的连续操作, 比如通过调用若干个接口实现一个完整功能, 这种我们将来会在API目录同级别的地方建立domain目录, 来存放这类用户操作.
