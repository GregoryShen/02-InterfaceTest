...
例4 我们调用了`repos.traffic` 下面的 `list_clone` 方法， 让 github 告诉我们某个用户名下的某个 repo 被 clone 过几次
...
把关键字层的文件放在operations 目录下。这里命名为 operations 直译是操作。 意思是用户的一系列操作。 也就是说，<u>关键字层是为了完成对==一系列用户操作的封装==。</u>这个命名是借鉴了快速测试中的SFDIPOT测试分析方法中的O
