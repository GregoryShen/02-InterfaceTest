...
| 请求行      | 方法 URL 版本, 比如 GET http://www.baidu.com HTTP1.1 |
| ----------- | ---------------------------------------------------- |
| Header 头部 | 字段名: 字段值...                                    |
| Body 请求题 | 具体业务数据, 比如一个json                           |
...
requests 库在引入后, 直接用“requests.方法名” 就可以发出对应方法的HTTP 请求. 而每个方法对应的参数, 第一个参数固定是请求头里的url, 之后的参数各有不同.
